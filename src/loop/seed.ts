...
export interface IterationSeed {
  ...
  northStar: { text: string; source: string };
  /** The mutable loop-state digest from the brain's compact rollup. */
  currentState: { text: string; source: string };
...
async function readFirst(root: string, rel: string): Promise<string | null> {
...
async function latestSpec(root: string): Promise<{ text: string; source: string } | null> {
  const specsDir = path.join(root, "specs");
  if (!(await pathExists(specsDir))) return null;
  try {
    ...
    for (const d of dirs.reverse()) {
      ...
      if (await pathExists(spec)) {
        ...
        if (text.length) return { text, source: `specs/${d}/spec.md` };
      }
    }
  } catch {
    /* fall through */
...
async function resolveNorthStar(root: string): Promise<{ text: string; source: string }> {
  for (const rel of NORTH_STAR_SOURCES) {
    ...
    if (text) return { text: firstSection(text, rel), source: rel };
  }
  return {
    source: "(none found)",
    text:
      "No north-star found. Create `.harness/SEED.md` with the stable goal / " +
...
function firstSection(text: string, rel: string): string {
  ...
  for (const line of lines) {
    ...
    if (isH && seenHeading) break;
    if (isH) seenHeading = true;
    out.push(line);
 ...
 */
export async function findBrainCompact(
  brainPath: string,
  repoName: string,
...
const NO_STATE =
  "No brain state yet. Once `commit-brain-agent` has written a compact rollup, " +
...
const FRESH_CONTEXT =
  "Re-establish fresh context every iteration: agents with " +
 ...
 */
export async function assembleSeed(opts: AssembleSeedOptions): Promise<IterationSeed> {
  ...
  let currentState: { text: string; source: string } = {
    text: NO_STATE,
...
export function renderSeed(seed: IterationSeed): string {
