...
export function currentEnv(): SchedulerEnv {
  return {
...
function memOf(agent: Subagent): number {
  return agent.resource_hint.max_memory_mb ?? DEFAULT_AGENT_MEMORY_MB;
}

function byPriority(a: Subagent, b: Subagent): number {
  return (
    (PRIORITY_RANK[a.resource_hint.priority] ?? 1) -
 ...
 */
export function calculateMaxParallel(
  readyAgents: Subagent[],
  env: SchedulerEnv = currentEnv(),
): ParallelPlan {
  ...
  const batch: Subagent[] = [];
  const deferred: Subagent[] = [];
  let memUsed = 0;
