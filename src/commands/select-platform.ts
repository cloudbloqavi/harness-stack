...
function validate(ids: string[], known: string[]): string[] {
  const unknown = ids.filter((id) => !known.includes(id));
  if (unknown.length > 0) {
    throw new Error(
      `Unknown platform(s): ${unknown.join(", ")}. Known: ${known.join(", ")}.`,
    );
  }
  return [...new Set(ids)];
}
...
export function parseSelection(raw: string, ids: string[]): string[] {
  const trimmed = raw.trim().toLowerCase();
  if (trimmed === "" ) return [ids[0]!];
  if (trimmed === "all" || trimmed === "*") return [...ids];
